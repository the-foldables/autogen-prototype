...
Your tool does not work for function prompting, only for structure and sequence prompts. The structure and sequence prompts must be the same length. The sequence prompt is a string of letters representing amino acids and the structure prompt is the coordinates of all the atoms in atom37 format as a PyTorch tensor. The unknown parts of the sequence prompt are '_' and the unknown parts of the structure prompt are None. The size of the structure prompt is the number of amino acid residues by 37 by 3. For example, for generating a protein with 4 amino acids, the sequence_prompt can be '____' and the structure_prompt can be [[[None]*3]*37]*4
You also need to specific the track, either 'sequence' or 'structure' and the num_decode_steps. 1 or 2 is a good value for num_decode_steps.
"""
